...
<Note>
  `{productId}` aceita o **UUID** do produto (recomendado — é o identificador estável e não-enumerável retornado por `GET /api/products/seller` e por `list_products` no MCP) **ou** o id numérico legado. Os dois funcionam; a partir da **v0.10.0** o UUID é o caminho preferido.
</Note>
