...
```bash cURL
curl -X GET "https://garu.com.br/api/products/seller?page=1&limit=10" \
  -H "Authorization: Bearer sk_test_sua_chave_api"
```

```javascript JavaScript
const response = await fetch(
  'https://garu.com.br/api/products/seller?page=1&limit=10',
  {
    headers: {
...
url = "https://garu.com.br/api/products/seller"
headers = {
...
async function listarTodosProdutos() {
  ...
  do {
    const response = await fetch(
      `https://garu.com.br/api/products/seller?page=${page}&limit=50`,
      {
        headers: {
          'Authorization': `Bearer ${process.env.GARU_API_KEY}`
        }
