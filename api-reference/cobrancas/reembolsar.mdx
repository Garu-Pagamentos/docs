...
<Note>
  Só cobranças confirmadas podem ser estornadas: `captured`, `payedPix` ou `payedBoleto`. Para cancelar uma cobrança que ainda **não** foi paga, use [`DELETE /api/v1/charges/{uuid}`](/api-reference/cobrancas/cancelar).
</Note>
...
|--------|-----------------|
| `400` | Cobrança não está em status estornável, já foi estornada, ou o valor excede o `chargedTotal` |
