...
```bash
curl -X POST "https://api.garu.com.br/api/portal/subscriptions/100/cancel?token={session_token}"
```

### Pausar Assinatura

```bash
curl -X POST "https://api.garu.com.br/api/portal/subscriptions/100/pause?token={session_token}"
```

### Retomar Assinatura

```bash
curl -X POST "https://api.garu.com.br/api/portal/subscriptions/100/resume?token={session_token}"
```

### Reativar Assinatura Cancelada

```bash
curl -X POST "https://api.garu.com.br/api/portal/subscriptions/100/reactivate?token={session_token}"
```

### Atualizar Método de Pagamento

```bash
curl -X POST "https://api.garu.com.br/api/portal/subscriptions/100/update-payment-method?token={session_token}" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "https://api.garu.com.br/api/portal/subscriptions/100/add-card-and-update?token={session_token}" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X GET "https://api.garu.com.br/api/portal/subscriptions/100/events?token={session_token}"
```
...
def abrir_portal():
    ...
    response = requests.post(
        'https://api.garu.com.br/api/portal/sessions',
        headers={
