...
const response = await fetch(
  'https://api.garu.com.br/api/subscriptions/100/events',
  {
    headers: {
...
url = "https://api.garu.com.br/api/subscriptions/100/events"
headers = {
...
async function getBillingHistory(subscriptionId) {
  const response = await fetch(
    `https://api.garu.com.br/api/subscriptions/${subscriptionId}/events`,
    {
      headers: {
        'Authorization': `Bearer ${process.env.GARU_API_KEY}`
      }
