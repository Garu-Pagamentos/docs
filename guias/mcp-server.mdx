...
```
Crie uma cobrança de R$ 297 no cartão em 6x para maria@email.com
```
...
```
Atualize o telefone do cliente cus_abc para (11) 98765-4321
```
...
<Accordion title="`permission denied` ao executar `npx`">
  ...
  Atualize o Node para 20.x ou superior e rode `npx --yes @garuhq/mcp --version`
  ...
  <Accordion title="O agente criou uma cobrança no ambiente errado">
    ...
    ser canceladas via dashboard ou via `cancel_charge` se ainda estiverem
    pendentes.
  </Accordion>
</AccordionGroup>
