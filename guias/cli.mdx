---
title: "CLI"
description: "Gerencie cobranças, carnês e reembolsos direto no terminal com a CLI da Garu"
---

## Visão Geral

A CLI da Garu permite criar cobranças, vender em carnê (boleto parcelado), gerenciar reembolsos e diagnosticar sua integração sem sair do terminal. Ideal para automações, scripts e agentes de IA que preferem trabalhar com comandos.
...
| `garu charges get <uuid>`    | Consulta detalhes de uma cobrança |
| `garu charges refund <uuid>` | Reembolsa uma cobrança            |

### Clientes

A partir da CLI 0.10.0 você gerencia clientes sem sair do terminal.

| Comando                                       | Descrição                                                 |
| ------------------------------------------------ | ---------------------------------------------------------------- |
| `garu customers create`                       | Cadastra um cliente para o seller atual                   |
| `garu customers list`                         | Lista clientes, com busca e filtro `--status overdue`     |
| `garu customers get <uuid>`                   | Consulta um cliente                                        |
| `garu customers update <uuid>`                | Atualiza campos do cliente (parcial)                       |
| `garu customers set-billing-email <uuid>`     | Define o e-mail de cobrança fixo                            |
| `garu customers clear-billing-email <uuid>`   | Limpa o e-mail de cobrança fixo                             |
| `garu customers delete <uuid>`                | Desvincula o cliente do seller atual                        |
...
### Carnê (Boleto Parcelado)

A partir da CLI 0.9.0, venda em [carnê](/guias/carne) — um produto dividido em 2 a 12 boletos mensais — direto do terminal.

| Comando                                          | Descrição                                                          |
| -------------------------------------------------- | --------------------------------------------------------------------- |
| `garu installment-plans create`                  | Vende um produto em carnê para um cliente                         |
| `garu installment-plans list`                     | Lista carnês, com filtros de status, cliente e produto            |
| `garu installment-plans get <uuid>`              | Consulta um carnê e todas as suas parcelas                        |
| `garu installment-plans reissue <uuid> <n>`      | Emite segunda via de uma parcela vencida                          |
| `garu installment-plans postpone <uuid> <n>`     | Adia o vencimento de uma parcela                                   |
| `garu installment-plans mark-paid <uuid> <n>`    | Marca uma parcela como paga manualmente                            |
| `garu installment-plans cancel <uuid>`           | Cancela o carnê e os boletos ainda em aberto                       |
| `garu installment-plans request-refund <uuid>`   | Solicita o estorno de um carnê                                     |

### Reembolsos

A CLI não reverte boleto nem Pix automaticamente — esse fluxo é registrar o pedido, você devolver o valor por fora, e depois confirmar. Cartão continua com estorno automático via `garu charges refund`.

| Comando                          | Descrição                                             |
| ----------------------------------- | ---------------------------------------------------------- |
| `garu refund-requests list`      | Lista pedidos de estorno (carnê e Pix/boleto)         |
| `garu refund-requests get <uuid>` | Consulta um pedido de estorno                         |
| `garu refund-requests confirm <uuid>` | Confirma que o valor já foi devolvido (chame **depois** de devolver) |
| `garu refund-requests reject <uuid>` | Recusa o pedido — o carnê ou a cobrança seguem intactos |
...
### Confirmar um reembolso após devolver o valor
...
| `--value`                     | Preço **em Reais** (ex: `49.90` = R$ 49,90)                            |
...
<Note>
  `--value` (produtos) é informado **em Reais** — o mesmo que o dashboard e a API REST (ex: `49.90` = R$ 49,90). Não use centavos: `--value 4990` cadastraria um produto de R$ 4.990,00.
</Note>
...
<Note>
  Tanto `--amount` (em `scheduled-charges`) quanto `--value` (em `products`) são informados em **Reais** (`49.90`). Mesma unidade nos dois comandos.
</Note>
