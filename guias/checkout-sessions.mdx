...
curl -X POST https://garu.com.br/api/v1/products \
  ...
  -d '{
    ...
    "image": "https://exemplo.com/curso.png",
    "value": 297.00
  }'
```

Anote o `uuid` do produto retornado — é ele que você envia no campo `product_id` da checkout session. (O campo se chama `product_id` por histórico, mas espera o **UUID** do produto, não um id numérico.)
...
const response = await fetch('https://api.garu.com.br/api/checkout/sessions', {
  ...
  body: JSON.stringify({ product_id: 'a1b2c3d4-e5f6-7890-abcd-ef1234567890' })
});
```
