...
Use esta opção quando **você** dispara a recorrência pela API (ex.: o cliente já está cadastrado e você quer agendar a série de cobranças). Mande `pix_automatic` no array `methods` do [`POST /api/scheduled-charges`](/api-reference/cobrancas-agendadas/criar).
