...
A partir da CLI/SDK `1.x` (v1 pública), use `/api/v1/customers` — camelCase, chaveado por `uuid`. Veja a referência completa em [Clientes](/api-reference/clientes/criar).
...
Útil quando o cliente quer que cobranças venham para um e-mail específico (ex: financeiro@empresa.com.br). Veja [Definir e-mail de cobrança](/api-reference/clientes/definir-email-cobranca):
...
```bash
curl "https://garu.com.br/api/v1/customers?page=1&limit=20&search=maria" \
  -H "Authorization: Bearer sk_live_xxx"
```
