...
```bash cURL
curl -X POST https://garu.com.br/api/v1/products \
...
const response = await fetch('https://garu.com.br/api/v1/products', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://garu.com.br/api/v1/products',
    headers={
        'Authorization': 'Bearer sk_test_sua_chave',
        'Content-Type': 'application/json'
    },
...
## E se eu quiser o pagamento na minha própria tela?

Os passos acima levam o cliente para uma página hospedada pela Garu — é o caminho mais rápido e cobre a maioria dos casos. Se você precisa que o pagamento aconteça **dentro da sua interface**, com a sua marca do começo ao fim, troque o Passo 3 por uma chamada de cobrança:
...
A resposta traz o **código PIX copia-e-cola** em `pix.code`, que você renderiza como QR Code no seu front. Nada de redirecionamento.

<Card title="Checkout Transparente" icon="qrcode" href="/guias/checkout-transparente">
  O guia completo: PIX, boleto com PDF servido pela Garu, cartão e as implicações de PCI.
</Card>
...
<CardGroup cols={2}>
  ...
  <Card title="Checkout Transparente" icon="qrcode" href="/guias/checkout-transparente">
    Receba PIX, boleto e cartão dentro da sua própria interface.
  </Card>
  <Card title="Boleto Parcelado (Carnê)" icon="calendar-days" href="/guias/carne">
    Venda um produto em até 12 boletos mensais. Leia antes de ativar: o risco de
    inadimplência é seu, não da Garu.
  </Card>
</CardGroup>
